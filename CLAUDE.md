...
- `meta/argument_specs.yml` — present on newer roles (`linux_provision`,
  `linux_seafile_cli`, `zabbix_agent2`, `zabbix_monitored_host`); add one when touching or
  adding a role.
...
**Monitoring checks are feature-local.** The role that installs a feature ships its Zabbix
agent 2 `UserParameter` drop-in into `/etc/zabbix/zabbix_agent2.d/` and the matching
`/etc/sudoers.d/` rule (exact command line, `visudo -cf` validated); `roles/linux_seafile_cli`
is the reference. That directory and the `zabbix` user are defined by the zabbix-agent2
*package*, not by `roles/zabbix_agent2`, so this is a drop-in like sudoers or logrotate, not a
role calling a role, and it works whether the agent is installed before or after. Two rules
follow from how the agent works: a `UserParameter` is only a key→command map, executed when a
Zabbix item asks for the key, so the drop-in is shipped unconditionally (flag on by default) and
needs no "is the binary there" guard; and the reload (`zabbix_agent2 -R userparameter_reload`)
must be gated on the agent being active, since `-R` talks to the running process. Server-side
objects (templates, items, triggers, linking) are **not** managed by feature roles.
...
- **Most roles still have no molecule coverage** — only `linux_provision`,
  `linux_seafile_cli`, `linux_additional_filesystems`, `docker-ce`, `backuppc-client`,
  `backuppc-client-rsync`, `zabbix_agent2` and `zabbix_monitored_host` have a `molecule/`
  directory (`zabbix-server`
  and the other retired Zabbix roles live under `temp/old/`).
...
- Comments in tasks and defaults explain *why* (upstream quirks, ordering constraints,
  idempotency guards), not what the module does. Match that density when editing. Keep them
  concise; do not turn a reviewer's follow-up remark or an example they gave into a comment —
  a value chosen during review needs a comment only if the *reason* is non-obvious and will
  matter to the next reader.
- Templated files start with the literal header
  `# Generated by Ansible. Any local changes will be overwritten` (comment syntax of the
  target format). **Do not use `{{ ansible_managed }}`**: the `DEFAULT_MANAGED_STR` setting
  that feeds it is deprecated in ansible-core and removed in 2.23. The older hyphenated roles
  still carry a `Generated by Ansible at {{ lookup('pipe', 'hostname -f') }}` header; replace
  it with the literal one when such a role is picked up.
